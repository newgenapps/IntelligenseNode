...
class EmailValidator {
  ...
  static async verifyMailbox(local, domain, [mxRecord], timeout, log) {
    if (!mxRecord || /yahoo/.test(mxRecord)) {
      ...
      return ('Cannot verify due to missing or unsupported MX record', mxRecord);
    }

    return new Promise(resolve => {
      const socket = net.connect(465, mxRecord);
      let resTimeout;
      ...
      const messages = [
        `HELO ${domain}`,
        `MAIL FROM: <testverifier13@gmail.com`,
      ...
      resTimeout = setTimeout(() => {
        ...
        ret(`Mailbox: timed out (${timeout} ms)`);
      }, timeout);
    });
  }
}
